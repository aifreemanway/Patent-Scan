...
**Источники данных (anti-fab — не заявлять лишнего):** патентные данные идут ТОЛЬКО через **Rospatent PatSearch** (датасеты RU/CIS + US/EP/JP/CN). **Espacenet OPS / EPO API — НЕ подключён** (`EPO_KEY`/`EPO_SECRET` в env есть, но в коде не используются) — не упоминать Espacenet как живой источник в копи/отчётах. Tavily (web) и Wikipedia/Wikidata — вспомогательные источники только в Industrial Usage и harvesting Скрининга, не в патентном поиске.
...
| #83 | Обязательный CAVEAT ГОСТ Р 15.011-96 во всех 3 продуктах (on-screen + HTML-export) |
...
**Ждёт Vsevolod:** ротация `TIMEWEB_AI_KEY`; точный вход Самары (recall pass «б»); ЮKassa-ключи + оферта/НДС для биллинга; имена/темы для демо-рассылки.
