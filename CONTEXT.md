...
### Анти-ЛЛМ прогон внешних текстов (обязательно, Vsevolod 25.08.2026)

Любой текст, который увидит клиент, перед выкатом прогоняется через `/humanizer`: письма, строки `messages/ru.json` и `messages/en.json`, лендинг, блог, тексты отчёта, формы, ошибки, рассылка.

Гейт, не рекомендация: копи без отметки о прохождении в работу не берётся. Прогон делает владелец копи (обычно ap-marketing) ДО передачи в реализацию.

Отличать от anti-fab выше: тот блок про факты, этот про стиль. Humanizer не трогает цифры, номера патентов, названия источников и юр-формулировки — у согласий, оферты и Политики приоритет за текстом ap-poverenny, стиль правится только в подтверждённых им пределах.
