...
Примеры правильного формата (РАЗНЫЕ темы — формат, не содержание):
...
- "литий-ионный аккумулятор катод LiFePO4 деградация ёмкости циклирование"
- "lithium-ion battery LiFePO4 cathode capacity fade cycling degradation"
- "перовскитные солнечные элементы стабильность инкапсуляция"
- "perovskite solar cells stability encapsulation"
- "вертикальное земледелие LED светодиоды салат гидропоника"
- "vertical farming LED lighting lettuce hydroponics"
...
Целься в 3-5 таблиц, разрезающих тему с разных сторон. Примеры — формат меняется в зависимости от темы:

Тема «производители X на рынке Y»:
  1) Крупнейшие производители (компании × мощность/выручка/технология)
  2) Производственные мощности по странам (страна × объём, динамика, доля)
  3) Технологические маршруты (метод × сырьё, выход, цена)
  4) Патентная активность (держатель × число патентов, ключевые направления)

Тема «методы / технологии решения задачи Z»:
  1) Сравнение базовых методов (метод × точность, скорость, требования)
  2) Эволюция методов по годам (год/поколение × ключевые улучшения)
  3) Применимость к разным сценариям (сценарий × подходящий метод × ограничения)

Тема «продукт / материал X: рынок и применения»:
  1) Сегменты применения (сегмент × доля рынка, ключевые игроки)
  2) Свойства разных модификаций / марок (марка × параметры × применение)
  3) Регуляторика по регионам (регион × ограничения, нормативы)
