...
Верни СТРОГО валидный JSON:
{
  ...
  "conclusions": [
    {
      "text": "1-2 предложения СТРАТЕГИЧЕСКОГО вывода — НЕ повтор overview",
      "sourceRefs": [1, 7]
    }
    // 5-7 элементов; ОБЯЗАТЕЛЬНО покрыть категории:
    //   • доминирующие игроки / лидеры (кто контролирует рынок, какая доля)
    //   • стратегические риски (концентрация поставщиков, санкции, патентная блокада)
    //   • тренды и драйверы (что меняется, что движет отраслью в ближайшие 3-5 лет)
    //   • регуляторика и стандарты (политика, экспортный контроль, environmental compliance)
    //   • технологический outlook (какие подходы уходят, какие приходят)
  ],
  "caveats": [
...
ОБЯЗАТЕЛЬНО включи ОДНУ ТАБЛИЦУ tables-of-players — сравнение конкретных компаний/предприятий/поставщиков с количественными данными. Это критическая ось для коммерческого использования отчёта. Колонки = названия компаний из SOURCES (Параметр × CATL × BYD × Gotion × EVE Energy), строки:
- HQ / страна
